...
class BrainFunction:
    ...
    def train(self, batch_size):
        ...
        actions_mask = np.ones((batch_size, ) + self.output_shape)
    ...
    def remember(self, state, proprioception, action, reward, next_state, next_proprioception, done):
        if reward > 0:
            self.positive_memory.append(
                (state, proprioception, next_state, next_proprioception, action, reward, done))
            self.psize += 1
...
class DQNAgent:
    def __init__(self, state_size, action_size, proprioception_size=2):
        ...
        self.brain = BrainFunction(model_builder = self._build_model, input_size = 2, 
    ...
    def act(self, state, is_randomic = False, proprioception=[0,0,0,0,0]):
        ...
        if is_randomic or p <= self.epsilon:
            ...
            act_values = self.brain.predict([state, np.expand_dims(proprioception, 0), self.mask_actions])
