...
def get_first_frame(env, noop_action_index):
    done = True
    obs = None
    while done:
        env.episode_done = False
        obs, _, done, _ = env.step(noop_action_index)
    return obs, done
...
class MaxWrapper(Wrapper):
    ...
    def __init__(self, env):
        ...
        self.useRaycasting = env.useRaycasting
    ...
    def reset(self):
        ...
        if self.useRaycasting:
            ...
            obs, done = get_first_frame(self.env, noop_action_index)
            if done:
...
class ExtractLuminanceAndScaleWrapper(ObservationWrapper):
    ...
    def __init__(self, env):
        ...
        self.useRaycasting = env.useRaycasting
        if self.useRaycasting:
                self.observation_space = spaces.Box(low=0, high=6, shape=(10, 10), dtype=np.uint8)
        else:
