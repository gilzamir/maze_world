...
class BrainMap:
    def __init__(self, width, hight):
        self.position = np.zeros(2, dtype='float32')
        self.dir = math.pi/2.0

    def update(self, action):
        if action == 0:
            self.position[0] += math.cos(self.dir)
            self.position[1] += math.sin(self.dir)
        elif self.action == 2:
            self.position[0] += math.cos(self.dir) * 10.0
            self.position[1] += math.sin(self.dir) * 10.0
        elif self.action == 1:

class BrainFunction:
    def __init__(self, model_builder, mem_size = 20000):
        self.psize = 0
        self.nsize = 0
        self.ntsize = 0
    ...
    def train(self, batch_size, postask=None):
...
class DQNAgent:
    def __init__(self, state_size, action_size, proprioception_size=2):
        ...
        self.brain = BrainFunction(self._build_model, 10000)
    ...
    def _build_model(self):
        ...
        output = layers.Dense(ACTION_SIZE, activaition='relu')(hidden2)
    ...
    def act(self, state, is_randomic = False, proprioception=[0,0,0,0,0]):
        ...
        if is_randomic or p <= self.epsilon:
            ...
            act_values = self.brain.predict([state, self.mask_actions, np.expand_dims(proprioception, 0)])
