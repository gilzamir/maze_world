...
for i in range(MAX_EPSODES):
    ...
    action = 0
    frame, reward, _, info = env.step(action, NB_FRAMES_IGNORED)
    ...
    agent.to_target_dir = agent.calc_to_target_dir()
    ...
    while not is_done:
        ...
        frame, last_reward, is_done, info = env.step(agent.contextual_actions[action], NB_FRAMES_IGNORED)

        orientation = info['orientation']
        position = info['position']
        agent.score = score = info['score']
        agent.position = position
        agent.target_position = info['target_pos']
        agent.isWithKey = info['isWithKey']
        agent.energy = info['energy']
        ...
        performer.shared_memory[0].remember([initial_state, np.expand_dims(initial_proprioception, 0)], action, reward, [next_state, np.expand_dims(next_proprioception,0)], end_eps)
        agent.goal_test_counter += 1        

        if random.random() <= 0.005:
            d = 1 if LOSS_COUNTER[goal] == 0 else LOSS_COUNTER[goal]
        ...
        if (performer.global_step >= performer.MAX_RANDOM_STEPS):
        ...
        if not is_done:
            ...
            if agent.goal_test_counter >= 300:
                goal = agent.get_current_goal()
                if goal != prev_goal:
                    goal_test_counter = 0
    ...
    for g in range(agent.nb_goals):
        ...
        if performer != None:
            ...
            if performer.step > 0:
                ...
                d = 1 if LOSS_COUNTER[g]==0 else LOSS_COUNTER[g]
